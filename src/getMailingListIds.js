var listMailingLists = require('./listMailingLists');
...
module.exports = async function getMailingListIds(mailingListName) {
  const lists = await listMailingLists();
  if (lists.length === 0) {
    console.log('You have no mailing list.');
  } else {
    return lists.filter(({ name }) => name === mailingListName).map(({ id }) => id);
  }
};